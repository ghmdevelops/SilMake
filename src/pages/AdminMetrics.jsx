...
export default function AdminMetrics() {
  ...
  // Lucro do período.
  //
  // LIMITAÇÃO IMPORTANTE: o pedido guarda o preço de venda, mas NÃO o custo.
  // E não pode guardar: o pedido é gravado pelo navegador do cliente, então
  // o custo teria que estar no código da loja — ou seja, visível para
  // qualquer pessoa. Por isso o cálculo usa o custo ATUAL do catálogo. Se
  // você mudar o custo de um produto, o lucro histórico dele muda junto.
  //
  // Produtos sem custo cadastrado ficam de fora, e o quanto isso representa
    ...
    soldOrders.forEach((order) => {
      (order.items || []).forEach((item) => {
        ...
        if (cost === null || cost === undefined) {
          uncoveredRevenue += lineRevenue;
          if (item.name) missing.add(item.name);
          return;
        }

        coveredRevenue += lineRevenue;
        profit += lineRevenue - cost * Number(item.quantity || 0);
      });
    });

    const base = coveredRevenue + uncoveredRevenue;
    return {
      profit,
      coveredRevenue,
      uncoveredRevenue,
      // Percentual das vendas que tem custo cadastrado.
      coverage: base > 0 ? (coveredRevenue / base) * 100 : 0,
  ...
  // Reposição: responde "o que eu preciso comprar essa semana".
  //
  // A conta é simples de propósito: unidades vendidas ÷ dias do período dá a
  // velocidade diária; estoque ÷ velocidade dá quantos dias ainda restam.
  // Ordena pelo que acaba primeiro, não pelo que vende mais — um produto que
  // vende pouco mas está com 1 unidade é mais urgente que um campeão de
  // vendas com 80 em estoque.
  //
  // Só entra quem tem controle de estoque: sem número cadastrado não há o
  // que projetar.
  const restock = useMemo(() => {
    ...
    soldOrders.forEach((order) => {
      (order.items || []).forEach((item) => {
        if (!item.id) return;
        soldById.set(item.id, (soldById.get(item.id) || 0) + Number(item.quantity || 0));
      });
    });

    return products
      .filter(hasStockControl)
      .map((product) => {
        const sold = soldById.get(product.id) || 0;
        ...
        return {
          ...
          sold,
          perDay,
          // Sem venda no período não dá para projetar: Infinity mantém o
          // produto no fim da lista em vez de fingir uma previsão.
          daysLeft: perDay > 0 ? product.stock / perDay : Infinity,
          // Quanto comprar para cobrir os próximos 30 dias.
          suggested: perDay > 0 ? Math.max(0, Math.ceil(perDay * 30 - product.stock)) : 0,
        };
      })
      .filter((row) => row.sold > 0 || row.stock <= 3)
      .sort((a, b) => a.daysLeft - b.daysLeft)
      .slice(0, 8);
  }, [products, soldOrders, period]);
    ...
    <div className="admin-metrics">
      ...
      <div className="metrics-cards">
        ...
        <div className="metric-card">
          ...
          <strong className={profitData.profit < 0 ? "metric-negative" : ""}>
            {profitData.coverage > 0 ? formatPrice(profitData.profit) : "—"}
          </strong>
          <small>
            {profitData.coverage === 0
              ? "cadastre o custo dos produtos para ver"
              : profitData.coverage >= 99.5
                ? `margem de ${profitData.margin.toFixed(0)}%`
                : `margem de ${profitData.margin.toFixed(0)}% — cobre ${profitData.coverage.toFixed(0)}% das vendas`}
          </small>
        </div>
        <div className="metric-card">
      ...
      {/* Sem este aviso, o lucro pareceria a verdade completa quando na
          verdade ignora tudo que não tem custo cadastrado. */}
      {profitData.uncoveredRevenue > 0 && profitData.coverage > 0 && (
        <p className="metrics-warning">
          {formatPrice(profitData.uncoveredRevenue)} em vendas ficaram de fora do lucro por não
          terem custo cadastrado
          {profitData.missing.length > 0 && <> — ex: {profitData.missing.join(", ")}</>}.
        </p>
      )}
      ...
      <div className="metrics-grid">
        ...
        <section className="metrics-panel">
          <h3>
            Precisa repor
            <small> (pelo ritmo do período)</small>
          </h3>
          {restock.length === 0 ? (
            <p className="admin-status">
              Nenhum produto com controle de estoque vendeu no período — sem vendas não há ritmo
              para projetar.
            </p>
          ) : (
            <ul className="metrics-list restock-list">
              {restock.map((row) => {
                const urgent = row.daysLeft <= 7;
                const attention = row.daysLeft <= 15;
                return (
                  <li key={row.id}>
                    <span className="restock-name">{row.name}</span>
                    <span className="restock-numbers">
                      <span title="Em estoque agora">{row.stock} un.</span>
                      <span title="Vendidas no período">vendeu {row.sold}</span>
                      <strong
                        className={urgent ? "restock-urgent" : attention ? "restock-warn" : ""}
                        title="Dias até acabar, no ritmo atual"
                      >
                        {row.daysLeft === Infinity
                          ? "sem giro"
                          : `${Math.floor(row.daysLeft)} dia(s)`}
                      </strong>
                      {row.suggested > 0 && (
                        <span className="restock-suggested" title="Sugestão para cobrir 30 dias">
                          comprar {row.suggested}
                        </span>
                      )}
                    </span>
                  </li>

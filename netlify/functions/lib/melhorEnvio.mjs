...
export function getConfig() {
  ...
  return {
    ...
    // Token fixo gerado direto no painel (Área Dev → Tokens). Quando existe,
    // ele dispensa TODO o fluxo OAuth: nada de autorizar, guardar tokens ou
    // renovar. É o caminho mais simples para quem tem uma loja só, e é o que
    // está em uso hoje. O OAuth abaixo continua valendo como alternativa.
    directToken: process.env.MELHOR_ENVIO_TOKEN || "",
    environment,
...
export async function getValidAccessToken({ force = false } = {}) {
  // Token fixo do painel: tem validade longa e não usa refresh_token, então
  // não há o que renovar nem onde gravar. Sai antes de tocar no Blobs.
